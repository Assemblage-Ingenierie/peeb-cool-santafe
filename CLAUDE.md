...
9. 🟡 Auth Supabase branchée : login réel (`/login`), session par cookies
   ...
   `peebcoolsf_perfiles`, déconnexion fonctionnelle. **Toute l'appli est derrière
   le login** (mur dans `proxy.ts` : non connecté → redirigé vers `/login`, `/api`
   → 401). Les données passent toujours par `service_role` (snapshot) mais côté
   utilisateur connecté. Écritures/admin gardées par la vraie session ; RLS déjà
   verrouillée (anon = rien) = 2ᵉ couche. Bypass dev conservé (= tout visible en local).
